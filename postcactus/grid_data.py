    ...
    def evaluate_with_spline(self, x, ext=2, piecewise_constant=False):
        ...
        if piecewise_constant or (
            ...
            return apply_to_points(
                lambda p: self._nearest_neighbor_interpolation(p, ext=ext),
                x,
...
class HierarchicalGridData(BaseNumerical):
    ...
    def evaluate_with_spline(self, x, ext=2, piecewise_constant=False):
        ...
        # We collect all results in a new dictionary with keys the indices
        ...
        for (ref_level, comp), points_indices in level_comps.items():
            ...
            evaluated.update(zip(points_indices, evaluated_points))

        # Finally, we prepare the output array and overwrite it with the values
        # that we computed.
        ret = np.zeros(len(points_arr), dtype=self.dtype)
        for index, value in evaluated.items():
            ret[index] = value

        # Before returning, we have to reshape the array to the correct form.
        ret = ret.reshape(original_shape[:-1])
        return ret

        # # We check that points has exactly the dimensions of the data, in this
        # # case we simply evaluate on the point.
